...
from speaker_model import Autoencoder_onehot
from utils import SpeechDataset, build_speaker_list, read_arrays_from_txt, retrieve_speaker_list, save_list_to_txt, one_hot_accuracy
...
model = Autoencoder_onehot().to(device)
...
for epochi in range(400):
    ...
    for idx, (batch,fileName_list) in enumerate(train_loader):
        ...
        for si in range(batch_size):
            L_tar20[0,positions[si],si*n_sample:(si+1)*n_sample]=1 # 这其实对应着 one-hot 的标签
        ...
        loss_onehot = criterion(w,L_tar20.transpose(1,2).to(device)) # one-hot
        loss = loss_onehot + loss_autoencode
        
        loss.backward() # 反向传播算梯度
        optimizer.step() # 反向传播改参数
        acc_avg_train += one_hot_accuracy(w,L_tar20.transpose(1,2).to(device))
    ...
    #保存 模型
    torch.save(model.state_dict(), "speaker_recognition_one-hot.pth")
    
    # 验证
    model.eval()
    with torch.no_grad():
        acc_avg = 0 # 这是语音样本在 one-hot 的下匹配准确率
        for idx, (batch,fileName_list) in enumerate(val_loader):
            ...
            acc_avg += one_hot_accuracy(w,L_tar20.transpose(1,2).to(device))
        ...
        torch.cuda.empty_cache()
    print("loss_avg:",loss_avg,"acc_avg_train:",acc_avg_train,"acc_avg:",acc_avg)
